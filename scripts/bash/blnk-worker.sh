################################################################################################################################################
## This is a Bash script used for setting up and deploying a blnk-finance worker service for the Blnk project. It performs the following tasks:
...
docker run -d --name server --network blnk-network \
  ...
  jerryenebeli/blnk:${blnk_version} /bin/sh -c blnk workers
